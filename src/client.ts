...
const TuningEditorForm = (tuning: SerializableTuning) => html`
  <form @submit=${(e: Event) => {
    ...
    if (name.trim() && notes.trim().split(" ").length === 6 && tuning.id) {
      appActor.send({ type: "UPDATE_TUNING", input: { id: tuning.id, name, notes } });
    }
  }} class="p-3 bg-zinc-700 rounded my-2 space-y-4">
    <div>
      <label for="tuning-name-${tuning.id}" class=${labelClasses}>Tuning Name</label>
      <input id="tuning-name-${tuning.id}" name="tuning-name" type="text" class=${baseInputClasses} .value=${tuning.name} required />
    </div>
    <div>
      <label class=${labelClasses}>Notes (6th to 1st string)</label>
      <div class="grid grid-cols-6 gap-2">
        ${tuning.notes.split(' ').map((note, i) => html`<input type="text" name="note-${i}" class="${baseInputClasses} font-mono text-center" maxlength="2" .value=${note} required />`)}
      </div>
    </div>
    <div class="flex justify-end gap-2">
      <button type="button" @click=${() => appActor.send({ type: "CANCEL_EDIT_TUNING" })} class=${secondaryButtonClasses}>Cancel</button>
      <button type="submit" class=${primaryButtonClasses}>Save Changes</button>
    </div>
  </form>
`;
...
const TuningManager = (savedTunings: SerializableTuning[], editingTuningId: string | null) => html`
  ...
  <div class="space-y-2">
    ...
    return html`
        <div class="flex items-center justify-between p-3 bg-zinc-800 rounded">
          ...
          <div class="flex gap-2">
            <button @click=${() => { if (tuning.id) appActor.send({ type: "EDIT_TUNING", id: tuning.id }); }} class="${secondaryButtonClasses} h-8 px-3 text-xs">Edit</button>
            <button @click=${() => {
        if (tuning.id) {
...
appActor.subscribe((snapshot) => {
  ...
  render(TuningManager(selectSavedTunings(snapshot), selectEditingTuningId(snapshot)), tuningManagerContainer);
