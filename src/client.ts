...
const deletePatternEffect = (input: { id: string }) =>
    ...
    Effect.flatMap((response) => {
      if (response && response.error) {
        return Effect.fail(
          new ApiError({
            message:
              (response.error.value as any)?.error ?? "API error deleting pattern",
            cause: response.error,
          }),
        );
      }
...
const createTuningEffect = (input: { name: string; notes: string }) =>
...
const updateTuningEffect = (input: {
  id: string;
  name: string;
  notes: string;
}) =>
    ...
    Effect.flatMap((response) => {
      if (response && response.error) {
        return Effect.fail(
          new ApiError({
            message:
              (response.error.value as any)?.error ?? "API error updating tuning",
            cause: response.error,
          }),
        );
      }
...
const deleteTuningEffect = (input: { id: string }) =>
    ...
    Effect.flatMap((response) => {
      if (response && response.error) {
        return Effect.fail(
          new ApiError({
            message:
              (response.error.value as any)?.error ?? "API error deleting tuning",
            cause: response.error,
          }),
        );
      }
...
const deleteChordEffect = (input: { id: string }) =>
...
const machineWithImplementations = appMachine.provide({
  actors: {
    ...
    deletePattern: fromPromise(({ input }) =>
      Effect.runPromise(deletePatternEffect(input)),
    ),
    createChord: fromPromise(({ input }) =>
      Effect.runPromise(createChordEffect(input)),
    ),
    updateChord: fromPromise(({ input }) =>
      Effect.runPromise(updateChordEffect(input)),
    ),
    deleteChord: fromPromise(({ input }) =>
      Effect.runPromise(deleteChordEffect(input)),
    ),
    createTuning: fromPromise(({ input }) =>
      Effect.runPromise(createTuningEffect(input)),
    ),
    updateTuning: fromPromise(({ input }) =>
      Effect.runPromise(updateTuningEffect(input)),
    ),
    deleteTuning: fromPromise(({ input }) =>
      Effect.runPromise(deleteTuningEffect(input)),
    ),
  },
});
...
const selectEditingChordId = (s: AppSnapshot) => s.context.editingChordId;
const selectEditingTuningId = (s: AppSnapshot) => s.context.editingTuningId; // NEW: Selector for the editing tuning
...
// --- LIT-HTML TEMPLATES ---
const PatternEditor = (currentPattern: string) => html`<textarea
...
>
  ...
  >
    ${props.viewMode === "json" ? "Visual View" : "JSON View"}</button
  ><button
    class=${primaryButtonClasses}
...
const NewPatternDialog = (newPatternName: string) => html`<div
...
// NEW: A template for the tuning editor form
const TuningEditorForm = (tuning: SerializableTuning) => html`
  <form
    @submit=${(e: Event) => {
    e.preventDefault();
    const form = e.target as HTMLFormElement;
...
const TuningManager = (
  ...
  <div class="space-y-2">
    ...
    // MODIFIED: Conditionally render edit form or display view for tunings
...
const controlsContainer =
  document.querySelector<HTMLElement>("#controls-container");
...
const chordBankContainer =
  document.querySelector<HTMLElement>("#chord-bank-container");
const tuningManagerContainer = document.querySelector<HTMLElement>(
  "#tuning-manager-container",
);
if (
  !editorContainer ||
  !controlsContainer ||
...
appActor.subscribe((snapshot) => {
  ...
  render(
    ChordBank(
      selectSavedChords(snapshot),
      selectSavedTunings(snapshot),
      selectEditingChordId(snapshot),
    ),
    chordBankContainer,
  );
  // MODIFIED: Pass the editingTuningId to the TuningManager component
  render(
    TuningManager(
      selectSavedTunings(snapshot),
      selectEditingTuningId(snapshot),
    ),
    tuningManagerContainer,
  );
